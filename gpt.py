...
my_api_key ="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"
...
def analyze_resume(job_desc, resume, options):
    ...
    summary_question = f"Job requirements: {{{job_desc}}}" + f"Resume summary: {{{df_string}}}" + "Please return a summary of the candidate's suitability for this position (limited to 1000 words);'"
    summary = ask_openAI(summary_question)
    df.loc[len(df)] = ['Summary', summary]
    extra_info = "Scoring criteria: Top candidate ,universities, leading company experience , well-known skills, overseas background,company background ."
...
def analyze_str(resume, options):
    ...
    for i, option in tqdm(enumerate(options), desc="Fetching information", unit="option", ncols=1000):
        question = f"What is this candidate's {option}? Please return the answer in a concise manner., If not found, return 'Not provided'"
